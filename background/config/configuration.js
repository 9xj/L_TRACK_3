class Config {
    constructor() {
        ...
        this.regions = ["syd-1", "syd-2", "beta-1"];
    ...
    extractFilteringAndClassroomConfig(serverConfig){
        ...
        if(serverConfig){
            ...
            let offNetowrkConfig = serverConfig["off_network"];
            ...
            self.filter_off_network_enabled = offNetowrkConfig["filtering"] ? true : false;
            self. classroom_off_network_enabled = offNetowrkConfig["classroom"]["enabled"];
        }
    }
    ...
    retrieve_configuration(errorCallback, ...callbacks) {
        ...
        self.retrieve_userConfig((newUserIdentity = false) => {
            if (self.getCurrentUserInfo().user) {
                ...
                let requestDevice = self.parent_device ? self.parent_device : self.device_id;
